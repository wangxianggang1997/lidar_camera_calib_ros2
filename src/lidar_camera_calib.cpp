...
int main(int argc, char **argv) {
  ...
  rclcpp::Node::SharedPtr node = rclcpp::Node::make_shared("lidar_camera_calib");
  // Keep pcd, image and calib loading as separate node

  // TODO: Load params via camera and lidar structs
  // Declare parameters
  node->declare_parameter("image_file", "");
  node->declare_parameter("pcd_file", "");
  node->declare_parameter("result_file", "");
  node->declare_parameter("camera_matrix", std::vector<double>());
  ...
  node->get_parameter("result_file", result_file);
  node->get_parameter("camera_matrix", camera_matrix);
